...
mk.controllers.Multiplayer.prototype.keyswitch = function (v, down) {//v is the key value and down is a boolean whether it should be keydown or no
  var pressed = {},
    self = this,
    f1 = this.fighters[0],
    f2 = this.fighters[1],
  ...
  if(down){
    pressed[v] = true;
    move = self._getMove(pressed, mk.controllers.keys.p1, 0);
    self._moveFighter(f1, move);
    move = self._getMove(pressed, mk.controllers.keys.p2, 1);
    self._moveFighter(f2, move);
  } else {
...
mk.controllers.Multiplayer.prototype.key = function(key, duration){//key is key num, duration is how long until toggle keyup
  var game = this;
  
  game.keyswitch(key, true);
  setTimeout(function(){
    game.keyswitch(key, false);
  }, duration);
}
